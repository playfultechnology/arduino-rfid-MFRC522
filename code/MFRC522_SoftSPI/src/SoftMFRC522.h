#ifndef SOFTMFRC522_H
#define SOFTMFRC522_H
...
// Error code returned when communicating with MFRC522
#define MI_OK 0
...
class SoftMFRC522 {
	public:
	  SoftMFRC522(const uint8_t csPin, const int8_t mosiPin, const int8_t misoPin, const uint8_t sckPin, const int8_t rstPin = -1, const unsigned long delayUs = 1);
	  void  begin();
	  void  showCardID(uint8_t *id);
	  void  showCardType(uint8_t* type);
	  void  writeTo(uint8_t addr, uint8_t val);
	...
	private:
	  ...
	  int8_t _mosiPin;
	  int8_t _misoPin;
	  int8_t _rstPin; // Note int8_t to allow -1 values if unneeded
	  // delay for clock being high
	  unsigned long _delayUs;
